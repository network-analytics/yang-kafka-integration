...
- For pre-built jars download the required jars
  from [GitHub Releases](https://github.com/network-analytics/yang-kafka-integration/releases/tag/v0.0.5-7.3).
...
3. Check the produced jars
    1. yang-schema-registry-plugin: `yang-schema-registry-plugin/target/yang-schema-registry-plugin-0.0.5-7.3-shaded.jar`
    2. yang-json-schema-serializer:
       `yang-json-schema-serializer/target/kafka-yang-json-schema-serializer-0.0.5-7.3-shaded.jar`
    3. yang-cbor-schema-serializer:
       `yang-cbor-schema-serializer/target/kafka-yang-cbor-schema-serializer-0.0.5-7.3-shaded.jar`
...
3. Copy the yang-schema-registry-plugin-0.0.5-7.3-shaded.jar to Confluent's schema registry java libs directory
    ```shell
    cp yang-schema-registry-plugin/target/yang-schema-registry-plugin-0.0.5-7.3-shaded.jar ${CONFLUENT_DIR}/share/java/schema-registry/
    ```
...
```bash
mvn install:install-file -Dfile=<download_path>/kafka-yang-json-schema-serializer-0.0.5-7.3-shaded.jar -DgroupId=ch.swisscom -DartifactId=kafka-yang-json-schema-serializer -Dversion=0.0.5-7.3 -Dpackaging=jar
mvn install:install-file -Dfile=<download_path>/kafka-yang-cbor-schema-serializer-0.0.5-7.3-shaded.jar -DgroupId=ch.swisscom -DartifactId=kafka-yang-cbor-schema-serializer -Dversion=0.0.5-7.3 -Dpackaging=jar
```
