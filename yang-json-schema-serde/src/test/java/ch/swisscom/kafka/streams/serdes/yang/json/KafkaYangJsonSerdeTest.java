package ch.swisscom.kafka.streams.serdes.yang.json;
...
public class KafkaYangJsonSerdeTest {

  private static final String ANY_TOPIC = "any-topic";

  private static ObjectMapper objectMapper = new ObjectMapper();

  private static KafkaYangJsonSerde<YangDataDocument> createConfiguredSerdeForRecordValues() {
    SchemaRegistryClient schemaRegistryClient =
        new MockSchemaRegistryClient(Collections.singletonList(new YangSchemaProvider()));
    KafkaYangJsonSerde<YangDataDocument> serde = new KafkaYangJsonSerde<>(schemaRegistryClient);
  ...
  public void shouldRoundTripRecords() throws Exception {
    KafkaYangJsonSerde<YangDataDocument> serde = createConfiguredSerdeForRecordValues();
  ...
  public void shouldFailWhenInstantiatedWithNullSchemaRegistryClient() {
    assertThrowsExactly(
        ...
        () -> new KafkaYangJsonSerde<>((SchemaRegistryClient) null));
  }
}
