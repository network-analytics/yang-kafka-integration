...
package fr.insa.kafka.streams.serdes.yang.json;

import fr.insa.kafka.serializers.yang.json.KafkaYangJsonSchemaDeserializer;
import fr.insa.kafka.serializers.yang.json.KafkaYangJsonSchemaDeserializerConfig;
import fr.insa.kafka.serializers.yang.json.KafkaYangJsonSchemaSerializer;
...
public class KafkaYangJsonSerde<T> implements Serde<T> {
  ...
  public KafkaYangJsonSerde() {
    inner =
        Serdes.serdeFrom(
            new KafkaYangJsonSchemaSerializer(), new KafkaYangJsonSchemaDeserializer());
  }
  ...
  public KafkaYangJsonSerde(final SchemaRegistryClient client, final Class<T> specificClass) {
    ...
    inner =
        (Serde<T>)
            Serdes.serdeFrom(
                new KafkaYangJsonSchemaSerializer<>(client),
  ...
  private Map<String, Object> withSpecificClass(final Map<String, ?> config, boolean isKey) {
    ...
    if (isKey) {
      newConfig.put(KafkaYangJsonSchemaDeserializerConfig.YANG_JSON_KEY_TYPE, specificClass);
    } else {
      newConfig.put(KafkaYangJsonSchemaDeserializerConfig.YANG_JSON_VALUE_TYPE, specificClass);
    }
    return newConfig;
  }
}
