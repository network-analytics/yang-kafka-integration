...
public abstract class AbstractKafkaYangCborSchemaSerializer<T> extends AbstractKafkaSchemaSerDe {
  ...
  protected void configure(KafkaYangCborSchemaSerializerConfig config) {
    ...
    this.validate =
        config.getBoolean(KafkaYangCborSchemaSerializerConfig.YANG_CBOR_FAIL_INVALID_SCHEMA);
  }
  ...
  protected byte[] serializeImpl(
    ...
    try {
      ...
      headers.add("content-type", "application/yang-data+cbor".getBytes(StandardCharsets.UTF_8));
  ...
  protected void validateYangCborMessage(T object, YangSchema schema)
