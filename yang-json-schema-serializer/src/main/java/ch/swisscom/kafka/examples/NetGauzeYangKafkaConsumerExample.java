 ...
 * <ul>
 *   <li><b>Approach A</b> – Legacy path via Confluent Schema Registry.
 *       {@code KafkaYangJsonSchemaDeserializer} fetches YANG schemas from SR at runtime.</li>
 *   <li><b>Approach B</b> – YANG Library path via local disk cache.
 *       Enabled by setting {@code yang.library.cache.path}. The deserializer builds a
 *       {@link org.yangcentral.yangkit.model.api.schema.YangSchemaContext} from an RFC 8525
 *       {@code yang-lib.xml} on disk — no SR call needed at consumer time.</li>
 * </ul>
 *
 ...
 * </pre>
 *
 * <p>The yangkit-specific logic (parsing {@code yang-lib.xml} into a
 * {@link org.yangcentral.yangkit.model.api.schema.YangSchemaContext}) is handled
 * internally by {@code YangLibraryCacheBuilder} in yang-schema-registry-plugin and is
 * demonstrated standalone in {@code App5YangLibrary} in the yangkit repo.
 */
public class NetGauzeYangKafkaConsumerExample {
    ...
    /**
     * Build consumer props for Approach A (Confluent SR, no local cache).
     * Set {@code yang.library.cache.path} to blank to disable YANG Library mode.
     */
    ...
    /**
     * Build consumer props for Approach B (YANG Library local cache).
     * Setting {@code yang.library.cache.path} to a non-blank directory enables
     * YANG Library mode in {@code KafkaYangJsonSchemaDeserializer}.
     */
    ...
    public static void main(String[] args) throws Exception {
        ...
        Map<String, Object> props = useYangLibrary
                ? yangLibraryConsumerProps()
                : schemaRegistryConsumerProps();

        System.out.printf("Starting consumer in %s mode%n",
        ...
        try (KafkaConsumer<String, YangDataDocument> consumer = new KafkaConsumer<>(props)) {
            ...
            while (true) {
                ConsumerRecords<String, YangDataDocument> records =
                ...
                for (ConsumerRecord<String, YangDataDocument> record : records) {
                    ...
                    if (doc == null) {
                        ...
                        System.err.printf("[offset=%d] Record skipped (null document)%n",
                                record.offset());
                        continue;
                    }
    ...
    private static void handleDocument(YangDataDocument doc,
                                       ConsumerRecord<String, YangDataDocument> record) {
        int children = doc.getDataChildren() == null ? 0 : doc.getDataChildren().size();
        System.out.printf("[partition=%d offset=%d] YANG document: %d root node(s)%n",
